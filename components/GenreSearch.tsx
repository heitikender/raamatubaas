...
type Opt = { name: string; book_count: number };

export default function GenreSearch({ initialValue = '' }: { initialValue?: string }) {
  ...
  function apply(name: string) {
    ...
    if (hid) { hid.value = name; hid.form?.requestSubmit(); }
  }
  ...
  return (
    <div className="ss" ref={boxRef}>
      <input type="hidden" name="zanr" defaultValue={initialValue} ref={hiddenRef} />
      <input
        type="text"
        placeholder="Žanr (tipi ≥3 tähte)…"
        value={text}
        autoComplete="off"
      ...
      {open && opts.length > 0 && (
        <ul className="ss-list">
          {opts.map(o => (
            <li key={o.name} onMouseDown={() => apply(o.name)}>
              <span>{o.name}</span>
      ...
      {open && text.trim().length >= 3 && opts.length === 0 && (
        <ul className="ss-list"><li className="muted">Žanrit ei leitud</li></ul>
      )}
    </div>
  );
}
